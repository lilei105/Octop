...
CREATE TABLE bio_script_folders (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
...
CREATE INDEX idx_bio_scripts_folder ON bio_scripts(folder_id);
CREATE INDEX idx_bio_scripts_avail ON bio_scripts(is_active, verified);

CREATE TABLE bio_analysis_tasks (
  id TEXT PRIMARY KEY,
  ...
  exec_finished_at INTEGER,
  exec_exit_code INTEGER,
  exec_error TEXT NOT NULL DEFAULT '',
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
CREATE INDEX idx_bio_tasks_agent_thread ON bio_analysis_tasks(agent_id, thread_id);
CREATE INDEX idx_bio_tasks_user ON bio_analysis_tasks(user_id);
